    ...
    <div className="flex flex-col h-full bg-transparent relative">
      ...
      {reactionDetails &&
        ...
        createPortal(
          ...
          >
            <div
              className="bg-[#1c1c1c] border border-[#333] rounded-2xl w-full max-w-xs shadow-2xl overflow-hidden flex flex-col animate-scale-in"
            ...
            >
              <div className="flex justify-between items-center px-4 py-3 border-b border-[#333] bg-[#111]">
                ...
                <button
                  onClick={() => setReactionDetails(null)}
                  className="text-slate-400 hover:text-white transition-colors p-1 rounded-lg hover:bg-[#333]"
                >
                  <X size={16} />
                </button>
              </div>
              <div className="p-3 max-h-60 overflow-y-auto custom-scrollbar flex flex-col gap-3 bg-[#111]">
                {Object.entries(reactionDetails).map(([emoji, users]) => {
                  if (users.length === 0) return null;
                  return (
                    <div key={emoji} className="flex flex-col gap-1">
                      <div className="text-sm border-b border-[#262626] pb-1.5 mb-1 flex items-center gap-2">
                      ...
                      {users.map((userId) => {
                        ...
                        return (
                          <div
                            key={userId}
                            className="flex items-center gap-2.5 px-2 py-1.5 hover:bg-[#222] rounded-lg transition-colors"
                          >
                            {avatarUrl ? (
                              <img
                                src={avatarUrl}
                                alt={displayName}
                                className="w-6 h-6 rounded-full object-cover bg-slate-700 ring-1 ring-[#333]"
                              />
                            ) : (
      ...
      {previewMedia &&
        ...
        createPortal(
          <div
            className="fixed inset-0 z-9999 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-fade-in"
            onClick={() => setPreviewMedia(null)}
          >
            <div
              className="relative max-w-full max-h-full flex flex-col items-center animate-scale-in"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                onClick={() => setPreviewMedia(null)}
                className="absolute -top-11 right-0 p-2 text-white/80 hover:text-white bg-[#222] hover:bg-[#333] border border-[#333] rounded-full transition-colors"
              >
                <X size={20} />
              </button>
              ...
              {previewMedia.type.startsWith("image/") ? (
                <img
                  ...
                  className="max-w-[90vw] max-h-[82vh] object-contain rounded-xl shadow-2xl border border-[#333]"
                />
                ...
                <video
                  ...
                  controls
                  autoPlay
                  className="max-w-[90vw] max-h-[82vh] rounded-xl shadow-2xl border border-[#333]"
                />
              ) : null}
              ...
              <a
                ...
                className="mt-3 text-xs text-slate-300 hover:text-blue-400 transition-colors"
              >
                      ...
                      >
                        ...
                        >
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-blue-400 hover:bg-[#333] rounded-lg transition-colors"
                          >
                            <Reply size={14} />
                          </button>
                        ...
                        {msg.publicUrl &&
                          (msg.fileType?.startsWith("image/") ||
                            ...
                            <div
                              className="cursor-pointer overflow-hidden rounded-xl relative group/media shadow-sm max-w-full border border-[#333]"
                              onClick={() =>
                                setPreviewMedia({
                                  url: msg.publicUrl!,
                                  type: msg.fileType!,
                        ...
                        {msg.publicUrl &&
                          ...
                          !msg.fileType?.startsWith("video/") && (
                            <a
                              ...
                              className="flex items-center gap-2 px-3 py-2 rounded-xl transition-colors bg-[#222] border border-[#333] hover:bg-[#2a2a2a] shadow-sm max-w-full"
                            >
                              <FileText
                                size={16}
                                className={
                                  isMe ? "text-blue-400" : "text-sky-400"
                                }
                              />
                          ...
                          Object.values(msg.reactions).some(
                            ...
                            >
                              ...
                              <button
                                ...
                                className="p-0.5 text-slate-500 hover:text-slate-300 hover:bg-[#333] rounded-full transition-colors"
                              >
                                <Info size={11} />
                              </button>
                            </div>
      ...
      <div className="shrink-0 flex flex-col gap-1.5 relative">
        ...
        {!canChat && (
          <div className="absolute inset-0 z-10 bg-black/70 backdrop-blur-[2px] flex flex-col items-center justify-center rounded-xl pointer-events-none border border-[#333]">
            <div className="bg-[#1c1c1c] text-slate-300 px-3.5 py-1.5 rounded-lg flex items-center gap-2 text-xs shadow-lg border border-[#333]">
        ...
        {/* Banner đang trả lời */}
        {replyingTo && (
          <div className="bg-[#1c1c1c] border border-[#333] px-2.5 py-1.5 rounded-lg text-[11px] text-slate-300 flex justify-between items-center">
            ...
            <button
              ...
              className="p-0.5 rounded-full hover:bg-[#333] text-slate-400 hover:text-white transition-colors shrink-0"
            >
              <X size={13} />
            </button>
          </div>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="relative">
            <select
              ...
              className="w-full appearance-none text-[11px] pl-2.5 pr-7 py-2 rounded-lg border bg-[#222] text-slate-300 border-[#333] focus:outline-none focus:border-blue-500/50 cursor-pointer transition-colors"
            >
          ...
          <form
            ...
            className="relative flex items-center rounded-xl border bg-[#222] border-[#333] px-1 py-0.5 focus-within:border-blue-500/50 transition-colors"
          >
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              disabled={!canChat}
              className="p-1.5 text-slate-400 hover:text-blue-400 disabled:opacity-40 transition-colors shrink-0"
            >
              <Paperclip size={16} />
            </button>
            <input
              ...
              className="flex-1 bg-transparent text-[13px] text-slate-200 px-1.5 py-1.5 focus:outline-none min-w-0 placeholder:text-slate-500"
            />
            <button
              type="submit"
              disabled={!inputValue.trim() || !canChat}
              className="p-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-40 disabled:bg-[#333] ml-0.5 transition-colors shrink-0 shadow-sm"
            >
              <Send size={14} />
            </button>
          </form>
