...
import { Users, Plus, Loader2, FolderOpen, Video, Settings, Search, X } from "lucide-react";

interface DashboardContentProps {
  initialRooms?: Room[];
}

export default function DashboardContent({ initialRooms = [] }: DashboardContentProps) {
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <aside className="w-[68px] bg-[#f8f9fa] border-r border-slate-200 flex flex-col justify-between items-center pt-0 pb-4 flex-shrink-0 z-20 shadow-[2px_0_8px_rgba(0,0,0,0.02)] relative">
          ...
          <div className="w-full flex flex-col items-center">
            ...
            <button className="flex flex-col items-center justify-center gap-1.5 w-full py-3 px-1 text-brand-600 bg-white border-l-2 border-brand-600 shadow-sm transition-all group">
              ...
              <span className="text-[10px] font-semibold tracking-wide">Teams</span>
            </button>
          </div>
            ...
            {showSettings && (
              <>
                ...
                <div className="absolute left-full bottom-0 mb-2 ml-4 w-56 bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.12)] border border-slate-100 py-2 z-50 flex flex-col text-[14px]">
                  ...
                  <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-2 text-red-600 hover:bg-red-50 transition-colors w-full text-left">
                    <span>{t("logout")}</span>
                  </button>
                </div>
              </>
        ...
        {!isLoading && (!rooms || rooms.length === 0) && (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            {!isLoading && rooms && rooms.length > 0 && filteredRooms && filteredRooms.length === 0 && (
              <div className="flex flex-col items-center justify-center py-24 gap-4">
                <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                  <Search className="w-6 h-6" />
                </div>
                <div className="text-center">
                  <h3 className="text-base font-bold text-slate-800">{t("no_results_title")}</h3>
                  <p className="text-xs text-slate-400 mt-1">{t("no_results_desc")}</p>
                </div>
              </div>
            )}
          </div>
