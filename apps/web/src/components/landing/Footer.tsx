    ...
    <footer className="bg-slate-50 border-t border-slate-200">
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-white rounded-[20px] p-7 border border-slate-200 shadow-sm min-w-[280px]">
              <div className="flex flex-col gap-5">
                <div>
                  <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{t('advisor_label')}</p>
                  <p className="text-[15px] font-bold text-navy">{t('advisor_name')}</p>
                </div>
                <div className="h-px w-full bg-slate-100" />
                <div>
                  <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{t('student_label')}</p>
                  <p className="text-[15px] font-bold text-brand-600 whitespace-pre-line">{t('student_name')}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="border-t border-slate-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 flex flex-col items-center justify-between gap-3 sm:flex-row">
          ...
          <p className="text-[13px] font-medium text-slate-400">{t('made_with')}</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-medium text-slate-500">{t('status')}</span>
          </div>
        </div>
      </div>
    </footer>
