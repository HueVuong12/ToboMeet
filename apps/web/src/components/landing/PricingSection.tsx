...
export default function PricingSection() {
  ...
  return (
    <section
      id="pricing"
      className="py-16 md:py-20 bg-white border-t border-slate-200/60"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-12">
          <div className="flex items-center gap-4">
            <span
              className={`text-[14px] font-semibold transition-colors ${!isYearly ? "text-navy" : "text-slate-400"}`}
            >
              {t("toggle_monthly")}
            </span>
            <button
            ...
            >
              <div
                className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform duration-300 ${isYearly ? "translate-x-7" : "translate-x-1"}`}
              />
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start max-w-4xl mx-auto">
          {PRICING_CONFIG.map((config) => {
            ...
            if (!tier) return null;
            const { icon: TierIcon, color: iconColor } = tierIcons[
              config.tier
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3
                    className={`text-[20px] font-bold tracking-tight ${config.highlighted ? "text-white" : "text-navy"}`}
                  >
                    {tier.name}
                  </h3>
                </div>
                ...
                {/* Description */}
                <p
                  className={`text-[14px] leading-relaxed mb-6 ${config.highlighted ? "text-slate-400" : "text-slate-500"}`}
                >
                  {tier.description}
                </p>
                ...
                <div className="mb-8">
                  {config.isFree ? (
                    <div
                      className={`text-[40px] font-bold tracking-tighter ${config.highlighted ? "text-white" : "text-navy"}`}
                    >
                      {t("free_label")}
                    </div>
                  ) : (
                    <div className="flex items-end gap-1">
                      <span
                        className={`text-[40px] font-bold tracking-tighter leading-none ${config.highlighted ? "text-white" : "text-navy"}`}
                      >
                        ${price}
                      </span>
                      <span
                        className={`text-[15px] font-semibold mb-1.5 ${config.highlighted ? "text-slate-400" : "text-slate-500"}`}
                      >
                        {t("per_month")}
                      </span>
                    </div>
                  ...
                  {isYearly &&
                    ...
                    config.yearlyPrice !== null &&
                    !config.isFree && (
                      <p
                        className={`text-[12px] mt-1 ${config.highlighted ? "text-emerald-400" : "text-emerald-600"}`}
                      >
                        {t("save_amount", {
                          amount: (
                            (config.monthlyPrice - config.yearlyPrice) *
                            12
                          ).toFixed(2),
                        })}
                      </p>
                ...
                {/* Divider */}
                <div
                  className={`h-px mb-6 ${config.highlighted ? "bg-white/10" : "bg-slate-100"}`}
                />
                ...
                <ul className="flex flex-col gap-3.5 flex-1">
                  {tier.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2.5">
                      ...
                      <span
                        className={`text-[14px] leading-snug ${config.highlighted ? "text-slate-300" : "text-slate-600"}`}
                      >
                        {feature}
                      </span>
                    </li>
