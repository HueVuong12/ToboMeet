...
function CalendarContent() {
  ...
  const [cmMemberQuery, setCmMemberQuery] = useState("");
  const [cmSuggestedUsers, setCmSuggestedUsers] = useState<any[]>([]);
  const [cmIsSearchingUsers, setCmIsSearchingUsers] = useState(false);
  ...
  // Click outside for room selection dropdown
  useEffect(() => {
    const handleClickOutsideRoom = (event: MouseEvent) => {
      if (
        roomDropdownRef.current &&
        !roomDropdownRef.current.contains(event.target as Node)
      ) {
        setRoomDropdownOpen(false);
        // Khôi phục text hiển thị khi click ra ngoài
        const selRoom = selectedRoomRef.current;
        const selChan = selectedChannelRef.current;
        if (selRoom && selChan) {
          setRoomSearchQuery(`${selRoom.name} > ${selChan.name}`);
        } else {
          setRoomSearchQuery("");
        }
  ...
  // Search users for channel meeting
  useEffect(() => {
    if (!cmMemberQuery.trim()) {
      setCmSuggestedUsers([]);
      return;
    }

    setCmIsSearchingUsers(true);
    const delayDebounceFn = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/users/search?q=${encodeURIComponent(cmMemberQuery.trim())}`,
        );
        if (res.ok) {
          const data = await res.json();
          const results = data && Array.isArray(data.result) ? data.result : [];
          setCmSuggestedUsers(results);
        }
      } catch (err) {
        console.error("Lỗi tìm kiếm user cho cuộc họp kênh:", err);
      } finally {
        setCmIsSearchingUsers(false);
      }
    }, 300);
            ...
            <form onSubmit={handleCreateChannelMeeting}>
              <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div className="relative" ref={roomDropdownRef}>
                  ...
                  {/* Combobox Search Trigger (Input gõ tìm kiếm trực tiếp) */}
                  <div className="relative">
                    <input
                      type="text"
                      value={(() => {
                        // Trạng thái 1: Đã chọn chính thức (Thêm 3 khoảng trắng cuối để con trỏ cách chữ ~15px)
                        const selectionText = selectedRoom && selectedChannel ? `${selectedRoom.name} > ${selectedChannel.name}   ` : "";
                        // Trạng thái 2: Nhập keyword
                        if (roomSearchQuery) {
                      ...
                      onChange={(e) => {
                        ...
                        if (selectionText && rawVal.startsWith(selectionText)) {
                          // Tách phần text gõ mới ở đằng sau phần đã chọn
                          const typedPart = rawVal.substring(selectionText.length);
                          // Nếu phần text gõ mới lại tiếp tục chứa trùng lặp với selectionText (do trình duyệt tự động điền hoặc click đúp), hãy bỏ qua phần trùng lặp đó
                          ...
                          // Nếu người dùng nhập đè từ đầu hoặc xoá bớt
                    ...
                    {selectedRoom ? (() => {
                      ...
                      return (
                        <div className={`w-5 h-5 rounded-md bg-gradient-to-br ${gradient} flex items-center justify-center text-white text-[9px] font-extrabold shadow-xs absolute left-3 top-2.5`}>
                  ...
                  {/* Dropdown Container dạng Tree view */}
                  {roomDropdownOpen && (
                    <div className="absolute left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-50 max-h-64 overflow-y-auto py-2">
                      {isLoadingRooms ? (
                        ...
                        return filtered.map((room) => {
                          // Nếu đang gõ search: Tự động mở rộng phòng khớp để user thấy kênh
                          // Nếu click tay: Sử dụng state đóng mở bình thường
                          const isExpanded = expandedRoomsInputMode ? true : (expandedRooms[room._id] ?? (selectedRoom?._id === room._id));
                          
                          // Sinh màu nền gradient ngẫu nhiên dựa trên room._id giống RoomCard
                          const CARD_GRADIENTS = [
                            "from-violet-600 via-purple-600 to-indigo-700",
                            "from-blue-600 via-indigo-600 to-cyan-700",
                            "from-teal-500 via-emerald-600 to-teal-800",
                          ...
                          return (
                            <div key={room._id} className="flex flex-col">
                              ...
                              {/* Danh sách Kênh thụt lề nếu phòng được expand */}
                              {isExpanded && (
                                <div className="pl-8 pr-4 py-1 flex flex-col gap-1 bg-slate-50/50">
                                  {(!room.channels || room.channels.length === 0) ? (
                                    ...
                                    room.channels.map((channel: any) => {
                                      // Nếu đã chọn chính thức kênh này, hoặc đang click vào một kênh mới của searchingRoom
                                      const isSelected = selectedChannel?._id === channel._id;
                                      return (
                                        <div
                                          key={channel._id}
                                          onClick={() => {
                                            // Click chọn kênh -> Lưu chính thức lựa chọn mới thay thế lựa chọn cũ và reset search
                                        ...
                                        >
                                          <input
                                            ...
                                            checked={isSelected}
                                            onChange={() => {}} // onClick handle trên thẻ div bọc ngoài
