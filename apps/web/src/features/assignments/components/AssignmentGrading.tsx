...
export default function AssignmentGrading({
  ...
  const handleSaveGrade = async () => {
    ...
    if (assignment.gradingType === "graded") {
      if (scoreInput.trim() === "") {
        toast.error("Vui lòng nh?p di?m");
        return;
      }
      score = Number(scoreInput);
      if (isNaN(score) || score < 0 || (assignment.maxScore !== undefined && score > assignment.maxScore)) {
        toast.error(`Ði?m ph?i n?m trong kho?ng t? 0 d?n ${assignment.maxScore}`);
        return;
      }
    }

    await onGrade(selectedSubId, score, feedbackInput);
    toast.success("Ðã luu di?m và feedback thành công");
  };
        ...
        <div className="w-80 border-r border-slate-200 bg-white overflow-y-auto flex flex-col shrink-0">
          <div className="p-4 border-b border-slate-100">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              Bài làm dã n?p ({submissions.length})
            </span>
          </div>
          ...
          {submissions.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-slate-400">
              ...
              <p className="text-xs font-semibold">Chua có bài n?p nào</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-[10px] text-slate-400">
                      <span>N?p: {new Date(sub.submittedAt).toLocaleDateString("vi-VN")}</span>
                      {sub.submissionStatus === "late" && (
                        <span className="text-amber-600 font-medium">Tr? {sub.lateMinutes} phút</span>
                      )}
                    </div>
                  </div>
                );
          ...
          {activeSubmission ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
                <div className="border-b border-slate-100 pb-3">
                  <h3 className="font-bold text-slate-800 text-sm">
                    Bài làm c?a {studentInfo?.displayName || activeSubmission.studentId}
                  </h3>
                  <p className="text-[11px] text-slate-400 mt-1">
                    N?p lúc {new Date(activeSubmission.submittedAt).toLocaleString("vi-VN")}
                  </p>
                </div>

                <div className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-slate-700">T?p bài làm</span>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
                ...
                {assignment.gradingType === "graded" && (
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between text-xs font-bold text-slate-700">
                      <span>Ði?m s?</span>
                      <span className="text-slate-400">Trên {assignment.maxScore}d</span>
                    </div>
                    <input
                      type="number"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-slate-700">Nh?n xét / Feedback</label>
                  <textarea
                    rows={4}
                    value={feedbackInput}
                    onChange={(e) => setFeedbackInput(e.target.value)}
                    placeholder="Vi?t nh?n xét c?a b?n..."
            ...
            <div className="h-[50vh] flex flex-col items-center justify-center text-slate-400">
              ...
              <p className="text-sm font-semibold">Ch?n h?c viên d? xem bài n?p và ch?m di?m</p>
            </div>
          )}
        </div>
      </div>
